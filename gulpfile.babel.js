...
import eslint, { formatEach, failAfterError } from `gulp-eslint`;
import { Server } from `karma`;
import { rmdirSync, mkdir, readFile as _readFile } from `fs`;
import sass, { logError } from `gulp-sass`;
import browserify from `browserify`;
import source from `vinyl-source-stream`;
import { createServer } from `http`;
import { commit, add, revParse, checkout, branch as _branch, merge, push } from `gulp-git`;
...
function lintJS() {
	return src([ `gulpfile.js`, SRC_JS_FILES, SPEC_FILES ])
		.pipe(eslint())
		.pipe(formatEach(`stylish`, process.stderr))
		.pipe(failAfterError())
		.on(`finish`, () => console.log(`ESLint: JavaScript code correct.`));
}
function testJS() {
	return new Promise((resolve, reject) => {
		let server = new Server(KARMA_CONFIG, e => { if(e) reject(e) });
		server.start();
		if( !KARMA_WATCH )
...
function buildCSS() {
	return new Promise((resolve, reject) => {
		src(`${SRC_CONTENT_FILES}.scss`)
			.pipe(sass().on('error', () => reject(logError)))
      .pipe(dest(DIST_DIR))
...
async function amendChanges() {
	await checkCommitRights();
	addFiles(`./`)
		.pipe(commit(null, { args: `--amend`, disableMessageRequirement: true, maxBuffer: Infinity })
			.on(`end`, async () => Promise.resolve(await publishChanges(),
...
function readFile(file, cb) {
	_readFile(file, (e, file) => e !== null ? cb(e) : cb(null, file));
}

// Git functions
function addFiles(path) {
	return src(path)
		.pipe(add({ quiet: true, maxBuffer: Infinity }));
}
function readBranch(cb) {
	revParse({ args: `--abbrev-ref HEAD` }, (e, branch) => e !== null ? cb(e) : cb(null, branch));
}
function checkoutBranch(branch, cb) {
	checkout(branch, e => cb(e));
}
function mkBranch(branch, cb) {
	_branch(branch, e => cb(e));
}
function mergeBranch(branch, cb) {
	merge(branch, { args: `--no-ff --log`, maxBuffer: Infinity }, e => cb(e));
}
function pushBranch(branch, cb) {
	push(`origin`, branch, { maxBuffer: Infinity }, e => cb(e));
}

const _default = defaultTask;
export { _default as default };
export const run = series( cleanDist, buildDist, buildApp, buildCSS, hostServer );
export const commit = commitChanges;
export const amend = amendChanges;
